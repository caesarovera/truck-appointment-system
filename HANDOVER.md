...
## Status
- Update terakhir: `2026-06-28` · Sesi: **Dashboard gate-officer** (`GET /gate/queue` + gate-in/out).
...
- Build frontend: `npm run test:js` → ✅ **49 pass** · `npm run type-check` (vue-tsc) → ✅ · `npm run build` → ✅.
...
1. **Persona terakhir — planner** (transporter ✅, driver ✅, gate-officer ✅): kelola slot window. `POST /slots` (buka) + `POST /slots/{id}/close` ada, tapi **belum ada endpoint list window** per gate/tanggal untuk planner (availability hanya status OPEN; planner perlu lihat semua termasuk CLOSED + utilisasi). Opsi: reuse `GET /reports/utilization` (sudah ada, planner-only, per window: capacity/booked/completed/no_show/cancelled/active) untuk halaman monitor; + FE form buka window & tombol close. Slice BE kecil bila perlu list window mentah.
2. **Lainnya:** wiring realtime (Reverb/Echo), laporan utilisasi company-scoped transporter, CRUD master data admin.
