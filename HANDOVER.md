...
## Status
- Update terakhir: `2026-06-28` · Sesi: **Frontend slice — Ketersediaan Slot** (TanStack Query pertama).
...
1. **Frontend slice berikutnya:** **booking** (form di halaman slot → `POST /appointments` + Idempotency-Key, invalidasi query availability). Lalu dashboard gate → jadwal driver → planner kelola window. Ketersediaan slot ✅ (slice 2). **Unblocker:** booking butuh pilih truk/sopir → perlu read endpoint armada transporter (`GET /me/fleet` trucks+drivers) & idealnya `GET /gates` untuk dropdown gate (ganti input angka). Pertimbangkan slice backend kecil ini dulu.
