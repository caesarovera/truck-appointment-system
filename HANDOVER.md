...
## Status
- Update terakhir: `2026-06-28` · Sesi: **Frontend — form booking** (`GET /me/fleet` → `POST /appointments`).
...
- Build frontend: `npm run test:js` → ✅ **24 pass** · `npm run type-check` (vue-tsc) → ✅ · `npm run build` → ✅.
...
1. **Frontend slice berikutnya:** pilih salah satu jalur fitur — (a) **jadwal driver** (`GET /me/appointments/today`), (b) **dashboard gate-officer** (gate-in/out, `POST .../gate-in|gate-out`), (c) **planner kelola window** (`POST /slots`, `/slots/{id}/close`), (d) **reschedule/cancel** dari detail appointment. Booking ✅ (slice 4). Pertimbangkan juga: halaman daftar appointment milik transporter (belum ada endpoint list appointment per company — mungkin perlu slice backend kecil).
