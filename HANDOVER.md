...
## Status
- Update terakhir: `2026-06-28` · Sesi: **Frontend — dropdown gate** (`GET /gates` di halaman availability).
...
- Build frontend: `npm run test:js` → ✅ **16 pass** · `npm run type-check` (vue-tsc) → ✅ · `npm run build` → ✅.
...
1. **Frontend slice berikutnya:** **booking** — form (pilih truk/sopir dari `GET /me/fleet` ✅, move_type, container_no/iso_type/size) → `POST /appointments` + Idempotency-Key → invalidasi query availability (`['slots-availability']`). Tampilkan `booking_code` saat sukses; map 409 (slot_unavailable/duplicate) & 422 ke pesan. Dropdown gate ✅ (slice 3). Lalu dashboard gate → jadwal driver → planner kelola window.
