<?php

declare(strict_types=1);

namespace App\Http\Controllers\Api\V1\Admin;

use App\Contracts\GateRepositoryInterface;
use App\Http\Requests\V1\Admin\AdminRequest;
use App\Http\Resources\V1\GateResource;
use App\Models\Gate;

final class ShowGateController
{
    public function __construct(private readonly GateRepositoryInterface $gates) {}

    public function __invoke(AdminRequest $request, Gate $gate): GateResource
    {
        return GateResource::make($this->gates->findForAdmin($gate->id));
    }
}
